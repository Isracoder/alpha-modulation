function [fx] = f_Audio_H0(x_states,theta_params,u_input,~)
    ...
    fx(1) = x_states(1); % take the previous posterior mean
    fx(2) = exp(x_states(2)); % previous posterior precision
    ...
    fx(1) = mu;
    fx(2) = log(fx(2)); % here we're just carrying over fx(2) from previous x(2), assuming fixed
