function [amp, phase] = hopf_oscillator(precision)
%% Kuramoto model in Cartesian coordinates (Hopf oscillators)
% clear; clc;

switch nargin
    case 0
        fprintf('no inputs provided, generating with default params')
        precision = 1 ;
        T = 30 ;
        dt = 0.001 ;
        plot = 1 ;
    ...
    case 1
        fprintf('using provided precision value of : %d \n', precision)
        % in this case use default precision and shorter time, assume this is for the g function
        T  = 2;                % simulation time, def = 30
        dt = 0.01;
        plot = 0 ; % don't plot at end
...
omega = 1 * 2 * pi;   % natural frequencies, def = 2 pi
